...
import TokenizeNftMint from './components/TokenizeNftMint'
import { getAlgodConfigFromViteEnvironment } from './utils/network/getAlgoClientConfigs'

/**
 * Tokenize Mint NFT Page
 * Page wrapper for uploading + minting an NFT-style ASA with local storage
 */
export default function TokenizeNftPage() {
  ...
  const onAssetCreated = () => {
    // NFTs are managed internally by TokenizeNftMint via localStorage
  }

  const onPrefillTransferAsset = () => {
    // Not used on this page but kept for component compatibility
  }
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-lg p-6 sm:p-8">
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h2 className="text-xl sm:text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Tokenize an NFT</h2>
                <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
                  Upload an image → backend pins to IPFS → mint an ASA with metadata on TestNet.
                </p>
